...
function MainPage() {
  ...
  const { midpoint, selection, game, result, isLoading, error } =
    useMainRoomSocket(roomUuid, myParticipantId);
  ...
  const canStart =
    participants.filter((participant) => participant.isReady === "Y").length >=
    1;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-header">
      <PageHeader
        title={<AppLogo className="mx-auto block h-11 w-11" />}
        showBack={false}
      />
      <RoomProgress
        step={
          result.data ? 3 : midpoint.data || game.modeVote || game.game ? 2 : 1
        }
      />
      <PageSheet
        className={
          ...
          <div className="flex flex-col gap-3">
            ...
            {isHost && (
              <Button
                ...
                fullWidth
                className="mt-1"
                onClick={midpoint.onFind}
                disabled={midpoint.isFinding}
              >
                {midpoint.isFinding ? "중간지점 찾는 중..." : "중간지점 찾기"}
              </Button>
            )}
            {isHost && midpoint.findError && (
              <ErrorMessage message={midpoint.findError} />
            )}
          </div>
        )}
      </PageSheet>
      {isChoosingRestaurant && (
        <FloatingConfirmBar
          isVisible={selection.pendingRestaurantId != null}
      ...
      {isChoosingMode && (
        <FloatingConfirmBar
          isVisible={game.pendingMode != null}
          onConfirm={game.onConfirmVote}
          disabled={game.isVoting}
        >
